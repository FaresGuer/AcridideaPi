...
def _ensure_container_data_schema_columns() -> None:
    ...
    with engine.begin() as connection:
        ...
        has_gas_level = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'gas_level'")
        ).first()
        if has_gas_level is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN gas_level FLOAT NULL"
                )
            )

        has_target_gas = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'target_gas_level'")
        ).first()
        if has_target_gas is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN target_gas_level FLOAT NULL"
                )
            )

        has_target_temp_min = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'target_temperature_min'")
        ).first()
        if has_target_temp_min is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN target_temperature_min FLOAT NULL"
                )
            )

        has_target_humidity_min = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'target_humidity_min'")
        ).first()
        if has_target_humidity_min is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN target_humidity_min FLOAT NULL"
                )
            )

        has_target_light_min = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'target_light_level_min'")
        ).first()
        if has_target_light_min is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN target_light_level_min FLOAT NULL"
                )
            )

        has_target_gas_min = connection.execute(
            text("SHOW COLUMNS FROM container_data LIKE 'target_gas_level_min'")
        ).first()
        if has_target_gas_min is None:
            connection.execute(
                text(
                    "ALTER TABLE container_data "
                    "ADD COLUMN target_gas_level_min FLOAT NULL"
                )
            )

        # If gas_level already exists and target is empty, seed target from current gas values.
        connection.execute(
            text(
                "UPDATE container_data "
